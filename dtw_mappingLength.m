...
x1=x1(:,1); % one channel
filename = sprintf('/Users/yijuwang/Desktop/spto/yw/song/s2.wav');
[x2, Fs]=audioread(filename);
x2=x2(:,1); % one channel
filename = sprintf('/Users/yijuwang/Desktop/spto/yw/song/s3.wav');
[x3, Fs]=audioread(filename);
x3=x3(:,1); % one channel
len = min([size(x1,1), size(x2,1), size(x3,1)]);
x=(x1(1:len)+x2(1:len)+x3(1:len))/3;
...
imagesc(frameTime,freq,log(abs(tempspec(n2,:))));
...
%slope_m=[];
num_file=4
filename = sprintf('%s%d.%s','/Users/yijuwang/Desktop/spto/yw/song/test/t/yw2_', num_file, 'wav');
info = audioinfo(filename);



% filename = sprintf('/Users/yijuwang/Desktop/spto/yw/song/test/t/yw1_11.wav');
% info = audioinfo(filename);
Fs = info.SampleRate;
...
[simi, locx, locy, path2]=sim(log(temp), log(dataspec));
%path=reshape(path,2,[])';
%plot(path(:,2),path(:,1))
simatrix=[simatrix, simi];
path = reshape(path2,2, [])';
%pathmatrix=[pathmatrix, ];
%end
 figure()
 plot(path(:,1),path(:,2))
len_tem = max(path(:,1));
t1 = floor (len_tem*0.1);
t2 = ceil (len_tem*0.9);
x1 = path(find(path(:,1)==t1,1),1);
y1 = path(x1,2);
x2 = path(find(path(:,1)==t2,1),1);
y2 = path(x2,2);
slope = (y2-y1)/(x2-x1);
...
s=10.^template; % template spectrogram
t=10.^data; % potential spectrogram
